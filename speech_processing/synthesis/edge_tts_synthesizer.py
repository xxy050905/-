...
class SpeechSynthesizer:
    ...
    def synthesize(self, text: str, output_path: str = None,
        ...
        print(f"[SYNTH-ENTRY] voice param={voice}, default_voice={self.default_voice}")
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(
                self._run_async, ssml, output_path, voice_name, speed_rate
            )
    ...
    def _run_async(self, ssml: str, output_path: str,
        ...
        try:
            loop.run_until_complete(
                self._synthesize_async(ssml, output_path, voice_name, speed_rate)
            )
        finally:
            loop.close()
    ...
    async def _synthesize_async(self, ssml: str, output_path: str,
        ...
        if is_translation_failed:
            ...
            print(f"[TTS-DEBUG-v2] Translation failed, using zh-CN-XiaoxiaoNeural")
        elif has_chinese:
            # 未翻译但有中文，使用中文语音
            voice = 'zh-CN-XiaoxiaoNeural'
            clean_text = text
            print(f"[TTS-DEBUG-v2] Has Chinese, using zh-CN-XiaoxiaoNeural")
        else:
            # 使用传入的 voice_name（而非 self.default_voice）
            voice = voice_name
            clean_text = text

        print(f"[TTS-DEBUG-v2] FINAL voice={voice}, rate={speed_rate}, text={clean_text[:80]}...")
